...
def pick_number():
    try:
        ...
        if fNum > sNum:
            result_label.config(text = "First Number must be less than Second Number.")
            return
        number = random.randint(fNum, sNum)
        result_label.config(text = f"Your pick is: {number}")
    except ValueError:
...
result_label = ttk.Label(content_frame, text = "", foreground = "#333", font = ("Segoe UI", 11, "bold"))
result_label.pack()
